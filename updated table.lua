...
-- ============================================
-- Wait for shared.muffin to be defined by config.lua
local timeout = 0
while not shared.muffin and timeout < 50 do
    task.wait(0.1)
    timeout = timeout + 1
end

if not shared.muffin then
    warn("Config not loaded! Make sure config.lua is loaded first.")
    return
end
...
-- Aggressive ESP refresh (every 2 seconds)
task.spawn(function()
    while task.wait(2) do
        if shared.muffin and shared.muffin['ESP'] and shared.muffin['ESP']['Enabled'] then
            pcall(function()
                refreshEsp()
                refreshTracers()
            end)
...
RunService.RenderStepped:Connect(function()
    if not shared.muffin then return end

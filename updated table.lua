...
local currentSmoothness = shared.muffin['Aim Assist']['Smoothness'] or 0
...
local function aimAt(part)
    ...
    if smoothness > 0 and smoothness < 1 then
        -- If we have a new target or smoothness changed, reset lerp
        ...
        -- Increment lerp progress by smoothness amount
        lerpProgress = math.min(lerpProgress + smoothness, 1)
        
        -- Smoothly interpolate between current and target CFrame
